...
class Blog extends Component {
      ...
      <main className="container py-md-20">
        ...
        <div className="row">
          ...
          <article className="col-12 col-md-8">
            ...
            <section>
              {/* {this.state.blogPage.map((item, idx) => ( */}
              <p className=" text-justify lh-lg mt-5 ">
            ...
            {/* <div className="mt-15 mb-8">
              ...
              {this.state.channelOther.map((item, idx) => (
                <NavLink to={`/channelBlog/${item.ChannelID}/blog/${item.BlogID}`} className="d-block fs-6 mt-8 mb-3 linkHover">
                  <i>
            ...
            <input
              type="button"
              className="btn btn-gradation btn-s px-4 hvr-bounce-in"
              style={{ float: "right" }}
              value=" 送出"
              onClick={this.mSend}
            />
  ...
  componentDidMount = async () => {
    ...
    let resultUser = await Axios.get(
      `http://localhost:9000/channelUserInfo/${document.cookie}`);
    this.setState({ userInfo: resultUser.data[0] })

    let newState = { ...this.state };
    newState.defaultMessenger[0].UserID = document.cookie;
